...
const long long C1 = 99997, C2 = 100001;
...
long long nextPrime2(long long n)
{
...
long long lastPrime2(long long n)
{
    for (long long i = n - 1;; i--)
    {
        if (isPrime2(i))
...
public:
    CustomProb1(long long n) : keysPresent(0)
    {
        // cout<<"in the constructor"<<endl;
        TABLE_SIZE = nextPrime2(n);
        PRIME = lastPrime2(TABLE_SIZE);
    ...
    void insert(string key, long long value)
    {
        ...
        while (hashTable[probe] != nullptr || hashTable[probe] == dummy)
        {
            ...
            probe = (temp + (i * offset * C1) + (C2 * i * i)) % TABLE_SIZE;
            i++;
        }
    ...
    bool search(string key)
    {
        ...
        while (true)
        {
            ...
            probeCount++;
            probe = (initialPos + (i * offset * C1) + (C2 * i * i)) % TABLE_SIZE;
            i++;
        }
...
public:
    CustomProb2(long long n) : keysPresent(0)
    {
        // cout<<"in the constructor"<<endl;
        TABLE_SIZE = nextPrime2(n);
        PRIME = lastPrime2(TABLE_SIZE);
